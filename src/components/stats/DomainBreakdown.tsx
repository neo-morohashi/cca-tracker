...
export function DomainBreakdown() {
  const { getLogsByDomain } = useStudyLogs();
  ...
  return (
    <div className="rounded-xl bg-slate-900 border border-slate-700 p-4">
      <p className="text-xs font-semibold text-slate-400 mb-3">ドメイン別累積時間</p>
              ...
              <div key={d.id} className="space-y-0.5">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-[11px] font-bold" style={{ color: d.color }}>{d.id}</span>
                    <span className="text-[10px] text-slate-400 truncate max-w-[70px]">{d.name}</span>
                  </div>
                  <span className="text-[10px] text-slate-400 shrink-0">{Math.round((d.value / total) * 100)}%</span>
                </div>
